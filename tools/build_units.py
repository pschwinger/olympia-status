...
<p>Most tools give you one of these. This production runs on all three, joined up, so what one practitioner learns the next one can use.</p>
<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(280px,1fr));margin:10px 0">
<div class="tile" style="border-left-color:#2FB89F"><b>Empirica &middot; the way of working</b><span>A careful method. The practitioner is honest about what it actually knows: it says when it <b>checked</b> something and when it is only <b>assuming</b>; it will not call a job done unless it really is; if it is unsure it says so, in writing, and never quietly fills the gap with a guess.</span></div>
<div class="tile" style="border-left-color:#6F6AAF"><b>Cortex &middot; the shared memory</b><span>Where everything is kept and shared between the practices: decisions, findings, open questions, the messages and files that move between storyboard, imagegen and videogen. Not documents you open; a memory the whole chain works from.</span></div>
<div class="tile" style="border-left-color:#f2c230"><b>Claude &middot; the practitioner</b><span>The part you talk to, steered in plain language. Each practice is its own seat with its own skills. It forgets each conversation, which is exactly why the shared memory exists.</span></div>
</div>
<div class="loop" style="margin-top:18px">
<div class="step" style="border-top-color:#f2c230"><b>1 &middot; You start</b><span>Philipp briefs a scene. The tools and the record are already there.</span></div><div class="arrow">&rarr;</div>
<div class="step"><b>2 &middot; It checks</b><span>Before acting it reads what is known: the sources, the registry card, the last ledger.</span></div><div class="arrow">&rarr;</div>
<div class="step" style="border-top-color:#6F6AAF"><b>3 &middot; It works</b><span>The spec, the frames, the film, the gate.</span></div><div class="arrow">&rarr;</div>
<div class="step"><b>4 &middot; It keeps</b><span>What was learned, what failed and why, so the next scene starts better.</span></div>
</div>
<p class="muted">The point is not any single step. It is the circle: what one practice learns, the next can find.</p>
